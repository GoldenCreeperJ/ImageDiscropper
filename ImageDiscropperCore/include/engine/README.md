...
> `src/engine/` 各阶段文件并**均已落地为真实实现**——切割线生成 / 网格铺设 / 切分 /
> 选择极性 / 排序 / 合成（分离·坍缩·重排）/ 导出（stb+libwebp 编码，分离多图写入文件夹）/ 配置 JSON 存取。
...
| `grid.h` | ② 诱导网格（§4.4） | `GridParams`、`RemainderPolicy`、`Cell`、`Grid`（`build` / `buildFromLines`） | ✅ 结构 + 铺设 |
...
| `composition.h` | ⑤ 排布导出（§5） | `EmitMode`、`MergeLayout`、`ExportFormat`、`CompositionParams`、`Composition`、`isCollapsible`、`compose` | ✅ 结构 + 判定 + 合成 |
...
| `image_io.h` | 图像编码 I/O（§5.5） | `writeImageFile` / `encodeImageToMemory`（底层 stb：PNG/JPEG/BMP；libwebp：WebP） | ✅ 已实现 |
...
> **格式支持**：导出编码支持 PNG/JPEG/BMP（stb）与 WebP（libwebp）四种格式，
