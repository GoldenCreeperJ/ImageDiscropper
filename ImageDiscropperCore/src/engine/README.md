...
——切割线生成与归一化 / 网格铺设与诱导 / 切分 / 选择极性 / 排序 / 合成（分离·坍缩·重排）/
导出（stb+libwebp 编码，多图写入文件夹）/ 配置 JSON 存取 / 顶层编排，覆盖终稿 §2 全流水线。
...
| `grid.cpp` | `Grid::build`（余量策略 DISCARD/KEEP_PARTIAL/PAD，E-5）+ `buildFromLines` + `induceGrid` | ② |
...
| `image_io.cpp` | stb 图像编码（PNG/JPEG/BMP）+ libwebp（WebP）；**全工程唯一定义 `STB_IMAGE_WRITE_IMPLEMENTATION`** | I/O |
