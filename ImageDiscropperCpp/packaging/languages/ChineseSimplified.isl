; Inno Setup ChineseSimplified.isl（6.7.1 兼容版）
; 来源：jrsoftware/issrc main 分支官方中文翻译，按键表过滤到 is-6_7_1 的 Default.isl。
; 生成方式：仅保留 6.7.1 编译器认识的键，未知消息不产生 ISCC 警告。
; 说明：Inno Setup 6.7.1（及全部已发布版本）不附带官方中文语言文件（官方中文仅在
; main 分支，尚未随任何发布版分发），故本仓库自带该文件供 installer.iss 引用，
; 避免依赖构建机的 Inno 安装内容。
