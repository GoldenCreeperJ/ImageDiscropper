...
// 分块依据：
//   - 本文件只承载“区域”这一数据概念（SPEC §2.2 术语表 / SPEC §2.3 基本形状），
//     不涉及切割线（cut_line.h）、区域集合（region_set.h）或网格（grid.h）。
//   - 坐标约定严格遵循 SPEC §2.1：整数像素、原点左上、x 向右 y 向下、
...
// ---------------------------------------------------------------------------
// RegionKind：区域种类，对应 SPEC §2.3 的五种基本形状。
...
struct RectRegion {
    ...
    // 是否为空：宽或高 <= 0 即视为空块，导出时应跳过（SPEC §3.3.3 贴边选框）。
