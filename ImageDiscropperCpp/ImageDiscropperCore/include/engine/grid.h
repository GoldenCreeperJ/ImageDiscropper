...
//       以及余量策略 RemainderPolicy，对应 Grid-Selection-Emit 阶段 ②（SPEC §1）
//       与 L3 网格分割模式（SPEC §3.4）。
// 分块依据：
//   - GridParams：网格定义参数（FR-L3.1）与余量策略（FR-L3.2），纯配置数据。
//   - Cell：一个单元格 = 一条竖带与一条横带的交集（SPEC §2.2），带行列号与序号。
...
// ---------------------------------------------------------------------------
// GridParams：网格定义参数（FR-L3.1 / SPEC §3.4.3）。
