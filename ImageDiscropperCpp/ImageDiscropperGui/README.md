...
>
> **配套文档**：最终用户操作手册见 [`USAGE.md`](USAGE.md)（§7.1 使用说明）；维护者设计文档见 [`DESIGN.md`](DESIGN.md)（§7.2 设计说明）。本文件聚焦目录职责与关键设计决策。
...
- **第五阶段 全局撤销重做 + 配置 + 文档与性能（进行中）**：已补齐 G-12（双历史 + 上下文路由的全局撤销/重做，编辑菜单+工具栏 Ctrl+Z/Y）、
  ...
  完整使用说明（[`USAGE.md`](USAGE.md)）与设计说明（[`DESIGN.md`](DESIGN.md)）已交付（§7.1/7.2）。
  剩余：8000×8000 ≥30fps 性能实测、逐项对照 §10 验收签署；已知缺口（箭头/扇形标注、网格线吸附、插值下拉、预处理全局撤销取舍）见 [`DESIGN.md`](DESIGN.md) §10。
