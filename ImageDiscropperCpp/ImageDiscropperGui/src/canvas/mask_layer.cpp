...
const QColor kKeepColor(0, 200, 0, 60);
const QColor kDeleteColor(200, 0, 0, 60);
...
// 依 Core 结果重建遮罩。
void MaskLayer::rebuild(QGraphicsScene& scene, const idc::engine::EngineResult& result,
    ...
    deleteBase_ = scene.addRect(0.0, 0.0, static_cast<qreal>(width), static_cast<qreal>(height),
    ...
    for (const auto& frag : result.kept.fragments()) {
        ...
        QGraphicsRectItem* item = scene.addRect(
            QRectF(static_cast<qreal>(r.left), static_cast<qreal>(r.top),
...
// 显隐切换（不重建）。
void MaskLayer::setVisible(const bool visible) {
