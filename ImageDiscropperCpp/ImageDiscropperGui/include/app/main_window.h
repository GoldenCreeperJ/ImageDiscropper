...
// 作用：主窗口（布局见本目录 README「布局结构与响应式规则」）——装配菜单栏、工具栏、状态栏、左侧面板、中央画布与
//       右侧参数/导出面板，并把各部件的信号槽串联为一条数据流：
//       用户交互 → Document（状态）→ EngineBridge（调 Core）→ CanvasScene（渲染）。
// 分块依据：
//   - MainWindow 只做「装配 + 编排 + 状态栏呈现」，不含切割/几何/导出实现（CONTRIBUTING.md「分层纪律」）；
//     真正的 Core 调用集中在 EngineBridge，状态集中在 Document。
//   - 构建（buildCentral/buildMenus/...）与响应（on* 槽）分组，避免上帝方法。
// 说明：覆盖打开图像/缩放平移、三种模式的切割遮罩与选区/单元交互、分离/坍缩/重排导出、降采样预览，
//       预处理（旋转/翻转/缩放/尺寸/黑白/反色/色道分离，图像处理面板），
//       标注工具与图层管理（AnnotationBridge 驱动 Core annotation/geometry，
//       画布矢量叠加渲染、导出可选烧录），以及全局撤销重做与配置加载/保存：
//       配置经 EngineBridge 委托 Core saveEngineConfig/loadEngineConfig，Document::applyEngineConfig 反向映射；
//       撤销重做采「双历史 + 上下文路由」——文档参数态用 HistoryManager<EngineConfig> 快照（防抖合并拖拽），
...
private slots:
    ...
    void onOpen();
    void onExport();
    void onLoadConfig();   // 文件→加载配置：读 JSON → Document::applyEngineConfig（入栈可撤销）。
    ...
    // 全局撤销/重做：文档参数态历史的防抖采集（连接 Document::changed 与定时器 timeout）。
...
private:
    ...
    void refreshPreview();
    // 依引擎结果刷新导出面板的输出预览缩略图（导出前预览）：缓存 res.ok/composition 后转 renderExportPreviewFromCache。
    ...
    // ---- 全局撤销/重做：文档参数态历史（双历史之一；标注历史在 AnnotationBridge/Core）----
    ...
    // ---- 全局撤销/重做：文档参数态历史（复用 Core HistoryManager，CONTRIBUTING.md「分层纪律」）----
    // 快照类型为 EngineConfig（与配置加载同构）：capture=buildEngineConfig、restore=applyEngineConfig，
