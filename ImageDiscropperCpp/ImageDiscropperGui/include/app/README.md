...
| 文件              | 声明                          | 职责                                                                                                                                                                                                                                                         |
|-----------------|-----------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `main_window.h` | `MainWindow`（`QMainWindow`） | 菜单 / 工具栏 / 状态栏 / 快捷键 + 连接 Document↔Canvas↔Panels 与 AnnotationBridge↔标注面板/画布手势的信号槽编排（持有 `AnnotationBridge annoBridge_`、`HistoryManager<EngineConfig> docHistory_`）；`refreshPreview` 为核心刷新入口；全局撤销/重做与配置加载/保存编排；导出输出预览只编排，渲染下沉 `util/output_preview_renderer` |
