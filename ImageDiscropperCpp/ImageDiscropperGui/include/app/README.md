...
| `preprocess_controller.h`  | `PreprocessController`（`QObject`）         | 预处理 8 操作编排（图像页面板与图像菜单共用同一批槽）：经 `EngineBridge` 调 Core `pixel_ops`，维度变化清失效选区/标注，耗时重采样包忙碌对话框 + 重入守卫；`setBusyOverlayMode` 供移动端改全屏遮罩（SPEC §8.2）                                                  |
