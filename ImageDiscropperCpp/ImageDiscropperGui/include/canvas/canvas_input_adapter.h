...
// 作用：ICanvasInputAdapter——画布输入适配器抽象接口（GuideLine 阶段 2「输入适配器重构」）。
//       把原始输入事件（桌面：鼠标 / 滚轮 / 右键；未来：触控）翻译为 CanvasView 的
...
// 说明：桌面实现 DesktopInputAdapter；触控实现 TouchInputAdapter（阶段 3：单指走
