...
// 作用：画布上的「选区框」图元——用户拖拽出的橙色矩形（本目录 README「画布视觉规范」：橙色实线 2px），
//       并把被标记的选区边向全图延伸绘制为橙色「贯穿切割线」。它是画布上唯一的橙色交互图元：
//       既能整体移动、四角手柄对角缩放，也能直接抓取任一条边（含其延伸到图像边界的那段）
//       沿法向拖动＝移动该边＝移动对应切割线（线与选区本为一体，绝不双重表示/错位/分离）。
//       移动/缩放/拖边时实时发出 rectChanged，驱动 Core 重算与遮罩即时刷新（NFR-6 / CONTRIBUTING.md「分层纪律」）。
// 分块依据：
//   - 本图元只处理「用户如何拖动这个框/这些边」的交互几何（移动/缩放/拖边/吸附），不含切割逻辑；
//     它产出的矩形交给 Document，再由 Core 决定诱导哪些贯穿切割线（CONTRIBUTING.md「分层纪律」）；
