...
，其余 GUI 代码只经 `EngineBridge` 间接使用引擎，不直接 include 引擎实现细节。

| 文件                    | 声明                                         | 职责                                                                                                                                                                  |
|-----------------------|--------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `document.h`          | `Document`（`QObject`）                      | 会话状态单一真相源：持有图像 / 模式 / 极性 / 选区 / L2 多矩形 / L3 网格与选择集 / 导出与重排参数，`buildEngineConfig` 组装、`applyEngineConfig` 反向还原，变更发 `changed()` / `imageChanged()`                     |
