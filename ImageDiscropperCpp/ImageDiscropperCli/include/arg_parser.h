...
// ---------------------------------------------------------------------------
// GlobalOptions：全局选项（布局见本目录 README「全局选项」）。可出现在子命令前后任意位置。
// ---------------------------------------------------------------------------
struct GlobalOptions {
    ...
    std::string config;      // --config <file>：从 JSON 配置读取全部参数（SPEC §7）。
...
//   - 其余选项：若紧随其后的 token 不以 '-' 开头，或为「负数值」（如 -5、-5,-7,10,20），
//     则作为其值消费；否则视为缺值（记录空串，交由命令层判定是否报错）。
