...
//       God File。与 Core 的 tests/test_harness.h 同构，
//       保持两个模块测试风格一致（CONTRIBUTING.md「分层纪律」）。
...
// 说明：不引入第三方测试框架（CONTRIBUTING.md「分层纪律」：CLI 依赖须与 Core 一致，仅用标准库）；
...
//   testArgParser   —— 单元：ArgParser 语法归类 + parseGlobalOptions 全局选项。
//   testValueParser —— 单元：value_parser 全部“字符串 → 类型”解析与格式校验。
//   testIntegration —— 集成：IT-1~IT-21，进程内调用 cliMain，校验退出码 / 输出尺寸 / 像素顺序。
// ---------------------------------------------------------------------------
void testArgParser();
