...
QImage toQImage(const idc::core::Image& img) {
    ...
    const auto* bytes = reinterpret_cast<const uchar*>(img.data());

    // Core Image 为行主序、紧凑排列（无行填充），故 bytesPerLine = 宽 × 每像素字节数。
    switch (img.format()) {
        case idc::core::ImageFormat::RGBA: {
            // 构造只引用 Core 缓冲的临时 QImage，随即 .copy() 深拷贝脱离该缓冲。
            QImage view(bytes, w, h, w * 4, QImage::Format_RGBA8888);
            return view.copy();
        }
        case idc::core::ImageFormat::RGB: {
            QImage view(bytes, w, h, w * 3, QImage::Format_RGB888);
            return view.copy();
        }
        case idc::core::ImageFormat::GRAY: {
            QImage view(bytes, w, h, w, QImage::Format_Grayscale8);
            return view.copy();
        }
    }
    return QImage();
