...
//       以及「工具」「图层」分组的占位（第二/四阶段接入）。面板是 Document 的控制器：
//       只把用户选择写回 Document，不含任何切割/几何逻辑（A-0.1）。
// 分块依据：模式与极性两个 QButtonGroup 各自映射到 Document 的一个字段；工具/图层占位
//           单独成组，便于后续阶段就地扩展而不动其它面板。
